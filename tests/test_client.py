...
from pepkio_ph_buffer_recipe_calculator.config import TOOL_ID
from pepkio_ph_buffer_recipe_calculator.exceptions import PepkioAPIError
from pepkio_ph_buffer_recipe_calculator.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "ph-buffer-recipe-calculator"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert "tris_50mM_1L" in names
        assert "phosphate_100mM" in names
        assert "stock_tris_dilution" in names
        inp = client.get_example_input("tris_50mM_1L")
        assert inp["mode"] == "calculator"
        assert inp["buffer_id"] == "tris"
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("tris_50mM_1L"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["mode"] == "calculator"
    assert captured["body"]["input"]["buffer_id"] == "tris"
    assert captured["body"]["options"]["label"] == "pytest"
    assert result.status == "completed"
    assert result.result["mode"] == "calculator"
    assert result.result["result"]["acid_mass_g"] == 3.0
